...
public class Battleship {
    ...
    public boolean playTurn(int c, int r) {
        ...
        // Will be added to boost numTurns if player successfully sinks oilTanker
        int explosionCtr = 0; 

        switch ((board[r][c].getHit())[firePos]) {
            case 0:
                board[r][c].setHit(firePos, 2);
                if (board[r][c].getShipType() == 1) {
                    // If the OilTanker has not been sunk yet, don't explode
                    if (board[r][c].checkSunkYet()) {
                        // If the ship is an OilTanker, explode everything around it within a
                        // 1-square radius
    ...
    /**
     * Gets the ship and position of specific square, and returns the position in
    ...
    private void positionShip(int shipType, Ship[][] board) {
        ...
        while (notYetPlaced) {
            ...
            if (board[bowRow][bowCol] != null || board[deckRow][deckCol] != null
                    || board[sternRow][sternCol] != null) {
                continue;
            }
